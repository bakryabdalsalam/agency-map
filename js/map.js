...
(function() {
    // Create and export a reference to the map if needed elsewhere
    ...
    window.createMarkers = function(properties) {
      window.markersCluster.clearLayers(); // Clear cluster group
      window.allMarkers = []; // Reset array
  
      properties.forEach((prop) => {
        const marker = L.marker([prop.location.lat, prop.location.lng]);
  
        // A popup with minimal info & buttons
        const popupHtml = `
